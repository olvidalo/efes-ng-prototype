...
A static website consists of plain HTML, CSS, and JavaScript files that can be served by any web server — no database, no application server, no runtime required. You can host a static site on GitHub Pages, Netlify, a university web server, or even a USB stick.

This is one of EFES-NG Prototype's key advantages over the original EFES: instead of requiring a running Java application server, the output is a folder of files you can deploy anywhere.
...
EFES-NG Prototype uses [Eleventy](https://www.11ty.dev/) (11ty) as its static site generator. Eleventy:
