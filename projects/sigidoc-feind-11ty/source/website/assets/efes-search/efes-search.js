...
var SearchEngine = class extends EventTarget {
  ...
  #index = null;
  #query = "";
  #facetFields = /* @__PURE__ */ new Set();
  #facetFilters = {};
  // field → Set<string>
  #dateRange = { from: null, to: null };
  #filtered = [];
  #fullFacetCounts = {};
  // computed once at load, defines value universe
  #facetCounts = {};
  // recomputed on each filter
  #status = "idle";
  #error = null;
  #url;
  #textFields;
  ...
  registerFacet(field) {
    ...
    if (!this.#facetFilters[field]) {
      this.#facetFilters[field] = /* @__PURE__ */ new Set();
    }
...
var EfesFacet = class extends HTMLElement {
  ...
  #build() {
    ...
    for (const [value, count] of sorted) {
      ...
      cb2.addEventListener("change", () => {
        this.#engine.setFacetValue(this.#field, value, cb2.checked);
      });
  ...
  #update() {
    ...
    const activeValues = this.#engine.activeFilters[this.#field] || /* @__PURE__ */ new Set();
    this.querySelectorAll(".efes-facet-item").forEach((item) => {
      const value = item.getAttribute("data-value");
      const count = counts[value] || 0;
      item.querySelector(".efes-facet-count").textContent = "(" + count + ")";
      const cb2 = item.querySelector("input");
      cb2.checked = activeValues.has(value);
      if (count === 0 && !cb2.checked) {
