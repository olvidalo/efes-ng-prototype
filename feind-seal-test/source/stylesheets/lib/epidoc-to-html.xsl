...
<!--
  EpiDoc-to-HTML wrapper for Feind Seal Test.

  Imports the project's start-edition.xsl from the sigidoc stylesheets,
  receives EpiDoc params from the pipeline config, and tunnels them to the
  body-structure template.

  Adjust the import path and body-structure template name if using a different
  stylesheet flavour.
-->
<xsl:stylesheet version="3.0"
    ...
    <xsl:param name="messages-file" select="''"/>

    <xsl:variable name="messages"
        select="if ($messages-file != '')
                then doc($messages-file)
                else if ($language != '')
